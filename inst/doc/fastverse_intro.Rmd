...
Using a config file in a project will ignore any global configuration as discussed in the previous section. You can still extend the *fastverse* inside a project session using `fastverse_extend`, or `options("fastvers.extend")` before `library(fastverse)`^[If you populate `options("fastvers.extend")` before calling `library(fastverse)`, with `_opt_fastverse.install = TRUE` set in `.fastverse`, the availability of these packages will also be guaranteed.]. 
